  ...
  public void execute() {
    ...
    if (hasTarget) {
      ...
      if (isValidTag) {
        ...
        if (shouldLog) {
          System.out.println("[AlignToAprilTag] ta: " + String.format("%.3f", ta) + "%, target: " + String.format("%.2f", targetArea) + "%");
          System.out.println("[AlignToAprilTag] Distance error: " + String.format("%.3f", distanceError) + "%");
          System.out.println("[AlignToAprilTag] PID raw output: " + String.format("%.3f", pidOutput));
        }
    ...
    m_driveSubsystem.driveRaw(xSpeed, 0, rotationSpeed);
